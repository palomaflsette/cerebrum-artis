"""
Script de treino V2 - IMPROVED VERSION

Melhorias sobre v1:
- ✅ Weighted loss (balanceia classes desbalanceadas)
- ✅ Fine-tuning parcial ResNet (layer4 unfrozen)
- ✅ Data augmentation (crop, flip, color jitter)

Expected: 72-75% accuracy (vs v1: 69.38%)

Usage:
    python train_v2.py --epochs 15 --batch-size 128 --multi-gpu
"""
...
# Import v2 modules
from multimodal_classifier_v2 import MultimodalEmotionClassifierV2
from dataset_v2 import create_dataloaders_v2, EMOTION_TO_IDX

ARTEMIS_EMOTIONS = list(EMOTION_TO_IDX.keys())
...
def compute_class_weights(train_loader):
    """
    Calcula pesos das classes baseado na frequência no dataset.
    Classes raras recebem peso maior.
    
    Returns:
        weights: Tensor (9,) com pesos normalizados
    """
    print("⚖️  Computing class weights...")
    
    # Count labels
    label_counts = torch.zeros(9)
    for batch in tqdm(train_loader, desc="Counting labels"):
        labels = batch['label']
        for label in labels:
    ...
    for idx, emotion in enumerate(ARTEMIS_EMOTIONS):
        print(f"      {emotion:15s}: {int(label_counts[idx]):6d} samples → weight {weights[idx]:.3f}")
...
def main(args):
    ...
    # Usar v2 dataloader com augmentation
    train_loader, val_loader, test_loader, tokenizer = create_dataloaders_v2(
        ...
        augment=True  # V2: augmentation ativado!
    )
    ...
    model = MultimodalEmotionClassifierV2(
        num_emotions=9,
        freeze_image_encoder='partial',  # V2: layer4 unfrozen!
    ...
    # V2: WEIGHTED LOSS (balanceia classes)
    class_weights = compute_class_weights(train_loader)
    criterion = nn.CrossEntropyLoss(weight=class_weights.to(device))
    print(f"⚖️  Weighted loss enabled (balancing classes)")
