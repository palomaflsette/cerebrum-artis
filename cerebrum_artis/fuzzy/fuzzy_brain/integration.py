...
# Import do deep-mind v1
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "deep-mind" / "v1_baseline"))
...
class HybridEmotionPredictor:
    ...
    def __init__(
        ...
        Args:
            neural_checkpoint_path: Caminho para checkpoint do deep-mind v1.
        ...
        # 3. Neural Model (deep-mind v1)
        if neural_checkpoint_path is not None:
            print(f"  🤖 Carregando deep-mind v1 de {neural_checkpoint_path}...")
            self.neural_model = self._load_neural_model(neural_checkpoint_path)
            
            # Transforms para o modelo (mesmo do treinamento)
            ...
            print("  ⚠️  Neural model não fornecido - usando apenas fuzzy!")
    ...
    def predict(
        ...
        Args:
            ...
            text: Utterance opcional para modelo neural (se None, usa texto vazio)
        ...
        Returns:
            ...
            Se return_components=True:
                Dict com:
                    ...
                    'neural': distribuição do deep-mind v1 (ou None)
        ...
        Example:
            ...
            ...     neural_checkpoint_path='/data/paloma/deep-mind-checkpoints/.../best.pt',
            ...     fusion_weight=0.7
            ... )
        ...
        # 2. Inferência neural (se disponível) - PRIMEIRO para guided fuzzy
        if self.neural_model is not None:
            p_neural = self._predict_neural(image_path, text=text)
        else:
            p_neural = None
        
        # 3. Inferência fuzzy (pode ser guiada por p_neural)
        # Se use_guided=True E p_neural disponível, fuzzy usa neural guidance
        if self.fuzzy_system.rules.use_guided and p_neural is not None:
            p_fuzzy = self.fuzzy_system.infer(features, p_neural=p_neural)
        else:
    ...
    def _load_neural_model(self, checkpoint_path: str):
        """
        Carrega modelo deep-mind v1 do checkpoint.
        
        Args:
            checkpoint_path: Caminho para checkpoint (.pt)
        
        Returns:
            Modelo deep-mind v1 carregado em eval mode (sem tokenizer)
        """
        try:
            # Carrega usando função helper do v1_baseline
            # Retorna (model, tokenizer), mas queremos só o model
            model, tokenizer = load_multimodal_classifier(
                checkpoint_path=checkpoint_path,
                device=self.device
            )
            
            # Salva tokenizer para uso posterior
            self.tokenizer = tokenizer
            
            model.eval()  # Modo inferência
            print(f"    ✅ Deep-mind v1 carregado com sucesso!")
            
            return model
            
        except Exception as e:
            warnings.warn(
                f"Erro ao carregar deep-mind v1: {e}\n"
                f"Retornando None (apenas fuzzy será usado)."
            )
            return None
    
    def _predict_neural(self, image_path: str, text: Optional[str] = None) -> Optional[Dict[str, float]]:
        """
        Prediz emoções usando o modelo neural deep-mind v1.
        
        Args:
            image_path: Caminho para imagem
            text: Utterance opcional (se None, usa texto vazio)
        
        Returns:
            Dict com 9 emoções [0,1] somando 1.0, ou None se falhar
        """
        ...
        try:
            ...
            image_tensor = self.transforms(image).unsqueeze(0)  # [1, 3, 224, 224]
            image_tensor = image_tensor.to(self.device)
            
            # 2. Tokeniza texto
            # Se text=None, usa string vazia (pior caso, mas permite inferência visual)
            # Se text fornecido, usa o utterance completo (MELHOR performance)
            if text is None:
                text = ""  # Fallback: texto vazio
            ...
            encoded = self.tokenizer(
                ...
                padding='max_length',
                truncation=True,
                max_length=32,  # Mesmo do treinamento
                return_tensors='pt'
            )
            
            input_ids = encoded['input_ids'].to(self.device)  # [1, 32]
            attention_mask = encoded['attention_mask'].to(self.device)  # [1, 32]
            
            # 3. Inferência
            with torch.no_grad():
                logits = self.neural_model(image_tensor, input_ids, attention_mask)  # [1, 9]
                probs = F.softmax(logits, dim=1)  # [1, 9]
            
            # 4. Converte para dict
            probs_np = probs.cpu().numpy()[0]  # [9]
            
            p_neural = {
                emotion: float(prob)
                for emotion, prob in zip(self.emotion_names, probs_np)
            }
            
            return p_neural
            
        except Exception as e:
            warnings.warn(f"Erro na predição neural: {e}")
            return None
    ...
    def explain_prediction(self, image_path: str) -> str:
        ...
        if result['neural'] is not None:
            explanation.append("\n🤖 PREDIÇÃO NEURAL (deep-mind v1 - multimodal ResNet+RoBERTa):")
...
if __name__ == "__main__":
    ...
    # Caminho do checkpoint v1
    checkpoint_path = "/data/paloma/deep-mind-checkpoints/multimodal_20251119_060954/checkpoint_epoch5_best.pt"
    
    # Cria preditor (neural + fuzzy)
    predictor = HybridEmotionPredictor(
