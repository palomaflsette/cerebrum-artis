...
class FuzzyInferenceSystem:
    ...
    def infer(
        ...
        p_neural: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
        ...
        Args:
            ...
            p_neural: Predição neural (opcional) para guided fuzzy.
                     Se fornecido, modula os outputs fuzzy baseado
                     na confiança neural. Dict com 9 emoções [0,1].
        ...
        Example:
            ...
            >>> # Guided fuzzy (v2.0)
            >>> p_neural = {'sadness': 0.85, 'awe': 0.05, ...}
            >>> emotions_guided = system.infer(features, p_neural=p_neural)
        """
        # 1. Valida inputs
        ...
        # 3. Se p_neural fornecido, aplica guided fuzzy (v2.0)
        if p_neural is not None and self.rules.use_guided:
            crisp_outputs = self._apply_neural_guidance(crisp_outputs, p_neural)
        
        # 4. Normalização → probabilidades
    ...
    def _apply_neural_guidance(
        self,
        crisp_outputs: Dict[str, float],
        p_neural: Dict[str, float]
    ) -> Dict[str, float]:
        """
        Aplica guided fuzzy: modula outputs fuzzy baseado em p_neural.
        
        ESTRATÉGIA (Neural-Guided Fuzzy v2.0):
        ---------------------------------------
        Para cada emoção, ajusta o output fuzzy baseado na confiança neural:
        
        1. SE p_neural[e] > threshold_alto (ex: 0.7):
           → AMPLIFICA fuzzy output (neural diz "sim" → fuzzy reforça)
           
        2. SE p_neural[e] < threshold_baixo (ex: 0.3):
           → ATENUA fuzzy output (neural diz "não" → fuzzy reduz)
           
        3. SENÃO (confiança média):
           → MANTÉM fuzzy output original
        
        Fórmula:
            crisp_guided[e] = crisp_fuzzy[e] * modulation_factor
            
        Onde modulation_factor ∈ [0.5, 2.0]:
            - p_neural[e] > 0.7 → factor = 1.5-2.0 (amplifica)
            - p_neural[e] < 0.3 → factor = 0.5-0.8 (atenua)
            - 0.3 ≤ p_neural[e] ≤ 0.7 → factor = 1.0 (mantém)
        
        Args:
            crisp_outputs: Saídas fuzzy originais [0,1]
            p_neural: Predições neurais [0,1]
        
        Returns:
            Saídas fuzzy moduladas [0,1]
        
        Example:
            crisp_fuzzy = {'sadness': 0.6, 'awe': 0.2, ...}
            p_neural = {'sadness': 0.85, 'awe': 0.05, ...}
            
            → sadness: 0.6 * 1.7 = 1.02 (clipped to 1.0)  # Amplificado!
            → awe: 0.2 * 0.6 = 0.12                       # Atenuado!
        """
        threshold_high = 0.7  # Alta confiança neural
        threshold_low = 0.3   # Baixa confiança neural
        
        crisp_guided = {}
        
        for emotion in self.emotion_names:
            fuzzy_value = crisp_outputs.get(emotion, 0.0)
            neural_conf = p_neural.get(emotion, 0.5)  # Default: neutro
            
            # Calcula fator de modulação
            if neural_conf > threshold_high:
                # Alta confiança neural → AMPLIFICA fuzzy
                # Map [0.7, 1.0] → [1.2, 2.0]
                factor = 1.2 + 0.8 * (neural_conf - 0.7) / 0.3
                
            elif neural_conf < threshold_low:
                # Baixa confiança neural → ATENUA fuzzy
                # Map [0.0, 0.3] → [0.5, 0.9]
                factor = 0.5 + 0.4 * neural_conf / 0.3
                
            else:
                # Confiança média → MANTÉM fuzzy
                # Map [0.3, 0.7] → [0.9, 1.2] (suave)
                factor = 0.9 + 0.3 * (neural_conf - 0.3) / 0.4
            
            # Aplica modulação
            guided_value = fuzzy_value * factor
            
            # Clip [0, 1]
            crisp_guided[emotion] = np.clip(guided_value, 0.0, 1.0)
